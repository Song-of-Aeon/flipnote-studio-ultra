function ntxt(wait_=0, sprite_=global.textguy[$"nothingman"].NOTHING, spritepos_=SPRITEPOS.HIDDEN, event_=c_null) {
	return { //NULL TXT, automatically progresses and has no text. use to set up or change sprites and whatever without needing to use a line
		action: function(tid) {
			tid.wait = real(wait);
			event();
			charpush(tid, new talkchar(sprite, spriteguy.char.name, spritepos));
		},
		event: event_,
		sprite: sprite_.val,
		spriteguy: sprite_,
...
function btxt(backgrounds=[], foregrounds=[], supergrounds=[], bglogic=c_null, fglogic=c_null, sglogic=c_null) {
	var me = {}; //BACKGROUND TXT, use to change background whatever dont care you get it
...
function ptxt(style=global.textstyles.nvl) {
	return { //PAGE TXT, use to start a new page in nvl mode or change the current textbox style
		action: function(tid) {
			tid.backlog = [];
			tid.tacklog = [];
			tid.x = style.x;
