...
nu texteffect("v", 2, u, function(argstr) { //(V)viggle, 2, xint yint
	//log(argstr);
	wiggle.x = easemult(ac_sin, gc+i, 120, real(string_copy(argstr, 1, 1)));
	wiggle.y = easemult(ac_cos, gc+i, 120, real(string_copy(argstr, 2, 1)));
...
nu texteffect("x", 2, u, function(argstr) { //(X)hake, 2, xint yint
	...
	shake.x = seed.next(xsize)-xsize/2;
	shake.y = seed.next(ysize)-ysize/2;
});
